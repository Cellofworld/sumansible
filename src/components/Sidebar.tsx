...
export default function Sidebar({ lessons, currentLesson, completedLessons, onSelect, isOpen }: SidebarProps) {
  return (
    <aside
      className={`fixed lg:sticky top-0 left-0 h-full w-72 bg-white border-r border-gray-200 z-40 transform transition-transform duration-300 lg:transform-none overflow-y-auto shadow-sm ${
        isOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
      }`}
      style={{ top: '0' }}
    >
      <div className="p-4 border-b border-gray-200 lg:hidden">
        <h2 className="font-bold text-lg text-gray-800">Содержание</h2>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full text-left px-3 py-2.5 rounded-lg flex items-center gap-3 transition-all duration-200 group ${
                isActive
                  ? 'bg-gradient-to-r from-red-50 to-orange-50 border border-red-200 text-gray-900 shadow-sm'
      ...
      <div className="p-4 mt-4 border-t border-gray-200">
        <div className="bg-blue-50 rounded-lg p-3 border border-blue-100">
          <p className="text-xs text-blue-600 font-medium mb-1">💡 Совет дня</p>
